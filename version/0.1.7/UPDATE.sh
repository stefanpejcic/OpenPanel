...
# Update from OpenPanel 0.1.6 to 0.1.7

# new version
NEW_PANEL_VERSION="0.1.7"
...
FUNCTIONS=(
    ...
    # add new values and defaults in the panel.config file
    extend_panel_conf_file
...
check_if_panel_services_started() {
    ...
    if echo "$admin_status" | grep -q "failed" || echo "$panel_status" | grep -q "failed"; then
        ...
        touch /etc/openpanel/upgrade/skip_versions
        echo "${CURRENT_PANEL_VERSION}" >> /etc/openpanel/upgrade/skip_versions
...
set_system_cronjob(){
    ...
    #remove previous
    grep -v -e 'opencli' -e '/usr/local/admin' -e 'systemctl reload nginx' /var/spool/cron/crontabs/root | tee /var/spool/cron/crontabs/root.tmp
