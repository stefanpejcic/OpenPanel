...
### 💅 Polish
- added *Edit Domain Templates* page under *OpenAdmin > Domains* for editing html templates and vhost files.
...
- bun, flarum, mautic and java are completely removed from PM2.
...
- added [Server Reboot](/docs/admin/system/reboot/) option in OpenAdmin.
...
- [Bug adding docroot in OpenPanel in Cyrillic - not IDN #431](https://github.com/stefanpejcic/OpenPanel/issues/431)
- [Delete email accounts on domain removal #233](https://github.com/stefanpejcic/OpenPanel/issues/233)
- fixed bug with `"` showing around enabled modules on *OpenPanel > Advanced > Server Info*.
- fixed error *Invalid folder path* on `/get-folder-size` for WP Manager.
- fixed bug setting load, du and cpu thresholds on *OpenAdmin > Notifications* settings.
...
- fixed bug on domain creation failing to create custom docroot.
- fixed bug connecting to SMTP locally from within the php containers.
- fixed a bug with ssh login notifications where the IP address was not correctly detected when multiple sessions were active.
