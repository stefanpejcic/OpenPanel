...
Tokens don't expire on their own. Revoke one any time from the same page if it's no longer needed or you think it leaked.
...
Every action Claude takes runs through the exact same `/api/` endpoints and permission checks as the panel UI and REST API — Claude only ever operates within your account and your plan's enabled features. If a request fails with a permission error, it usually means that feature isn't enabled
on your plan.
...
| Client can't connect / 401 | Token is wrong, was revoked, or the header isn't set correctly. |
