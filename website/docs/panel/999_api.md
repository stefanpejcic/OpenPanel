...
Write-only — there is no GET, since the web UI never displays the current root password either. Updates both `%` and `localhost` hosts, plus the stored `.env`/`my.cnf`, and restarts the service.
...
Two-step, since API auth is stateless bearer-token (no session cookie to stash a pending import in like the web UI does): the first call validates a CSV upload and returns an `import_token`; the second confirms it and creates the mailboxes.
...
`PUT` takes the domain in the body (not the path) — `PUT /api/php/domains/<domain>` would be ambiguous with `PUT /api/php/<version>/options` under Go's router, since both are three-segment patterns with the wildcard in a different position. Not available on LiteSpeed, which has no per-domain PHP — use `PUT /api/php/default` instead.
...
on. Enrolling a **passkey** isn't available over the API — WebAuthn is a
browser-native ceremony tied to a session cookie, not a bearer-token
client; use the panel UI at `/account/passkeys` for that. Listing and
revoking existing passkeys is available below.
...
```json
{"tokens": [{"id": 1, "name": "smoke-test", "token_prefix": "op_mcp_abc123", "read_only": true, "created_at": "2026-01-15 12:00:00"}]}
```
...
Read-only informational endpoints, always available regardless of `enabled_modules`.
...
The single-click "back up my whole account" flow — creates a `.tar.gz` of the account's docroot volume in the background via `opencli user-backup`.
...
All paths are relative to `/var/www/html/`. Every endpoint enforces the same path-containment guard as the web UI (`paths.SecureUserPath`) — a path that escapes the account's docroot is rejected regardless of how it's phrased.
...
`GET` returns the raw content as a bare JSON string (not wrapped in an object) — the same contract the web editor's own AJAX mode uses. Both enforce the extension allowlist / binary-file check the web editor uses; `PUT` also normalizes line endings for known text file types.
...
Download streams `application/octet-stream` with a `Content-Disposition` attachment header; view returns the raw content as `text/plain` (or serves the file directly for images). Both respect `path_param` as the parent directory query param and the same allowlist as the editor.
...
Fire-and-forget, same SSRF guard as the web UI (only public http/https hosts — no loopback/private/link-local targets). Returns a `download_id` to poll.
...
Creates the app (the endpoints above then manage it). Unlike the rest of this API, the request body is form-encoded (not JSON) and the response streams newline-delimited JSON status/error events as the install progresses, rather than a single JSON object — the same contract the web install form itself uses.
...
Shallow-clones the repo to a throwaway temp dir to guess the entry point (`package.json`'s `main`, or common filenames like `index.js`/`app.py`), then discards the clone.
