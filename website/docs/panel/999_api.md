...
Install and uninstall only — no listing, cloning, or hardening endpoints. Installed sites still show up in the general Site Manager (`GET /api/sites`).
...
Install and uninstall only. Installed sites still show up in the general Site Manager (`GET /api/sites`).
...
Deliberately simpler than the WordPress API below: install, clone, and uninstall only — no listing endpoint, no filesystem scan, no hardening rules. MySQL/MariaDB only. Installed sites still show up in the general Site Manager (`GET /api/sites`).
