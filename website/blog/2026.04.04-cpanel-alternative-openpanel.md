...
tags: [cPanel alternative, web hosting panel, OpenPanel, selfhosted hosting, VPS hosting panel]
...
Then [Oakley Capital acquired cPanel in 2018](https://www.oakleycapital.com/news-and-insights/oakley-capital-to-invest-in-cpanel). In 2019, they scrapped the unlimited model completely and switched to per-account pricing. Hosts with thousands of accounts saw their bills jump 500% overnight. The community was furious. Nothing changed.
...
And here's the part that should concern you: in 2020, [Oakley Capital also bought Plesk](https://www.oakleycapital.com/news-and-insights/acquisition-of-plesk) - cPanel's main competitor. Both are now owned by the same company, WebPros. There is no competitive pressure anymore. Prices will keep going up because there's no reason for them not to. 🤷🏻‍♂️
...
- **WordPress management.** The majority of hosted sites run WordPress. [Auto-installs, caching integration, debugging tools, and backup management](/docs/panel/applications/wordpress/) all need to work without custom scripting.
...
1. [**Install OpenPanel** on a new VPS](/install/)
2. **Generate full cPanel backups** for each account you're moving ([cPanel → Backup Wizard → Full Backup](https://docs.cpanel.net/cpanel/files/backup-for-cpanel/#generate-a-full-backup))
3. [**Create matching hosting plans**](/docs/admin/plans/hosting_plans/#create-a-plan) with equivalent resource limits
4. **Import the backups** directly into OpenPanel [via the admin interface](/docs/articles/transfers/import-cpanel-backup-to-openpanel/) or CLI
...
A few things worth knowing before you start: DNS propagation takes time regardless of which panel you're on, so plan the cutover during low-traffic windows. And if any accounts have custom cPanel-specific configurations (EasyApache-compiled modules, custom WHM hooks), those will need manual review as the backup import handles data, not bespoke server config.
...
Also, the Community Edition is free to install:
