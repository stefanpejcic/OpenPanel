...
func BuildSidebarNav(allowed map[string]bool, path string) []NavGroup {
	...
	if has("wordpress", "drupal", "joomla", "opencart", "website_builder", "nodejs", "python") {
		var links []NavLink
		...
		open := hasAnyPrefix(path, "/auto-installer", "/sites", "/website", "/wordpress", "/drupal", "/joomla", "/opencart", "/pm2", "/nodejs", "/python")
