...
type mediawikiBackupDateInfo struct {
...
func handleMediaWikiGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []mediawikiBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !mediawikiBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := mediawikiBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handleMediaWikiRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !mediawikiBackupDateRE.MatchString(backupDate) {
	...
	if _, statErr := os.Stat(targzPathOnHostOS); statErr == nil {
		...
		_ = logger.RecordUserAction(a.Config, currentUsername, "restored MediaWiki files backup from "+backupDatePathInContainer+" on "+selectedDomain, ipAddress)
	...
	if _, statErr := os.Stat(databaseSQLPathOnHostOS); statErr == nil {
		...
		if dbName != "" {
			...
			_ = logger.RecordUserAction(a.Config, currentUsername, "restored MediaWiki database backup from "+databaseSQLPathInContainer+" on "+selectedDomain, ipAddress)
...
// handleMediaWikiRunBackup mirrors wordpress/backups.go's handleRunBackup.
func handleMediaWikiRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	selectedDomain := r.PathValue("selected_domain")
	docroot := r.URL.Query().Get("docroot")
	if docroot == "" {
	...
	if backupDatabase {
		...
		if dbName == "" {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to determine database name from LocalSettings.php."})
			return
		}
	...
	case backupDatabase && backupFiles:
		logAction = "generated full backup for MediaWiki website " + selectedDomain
	case backupDatabase:
		logAction = "generated database backup for MediaWiki website " + selectedDomain
	case backupFiles:
		logAction = "generated files backup for MediaWiki website " + selectedDomain
	}
