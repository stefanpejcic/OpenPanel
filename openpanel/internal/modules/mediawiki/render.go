...
var installPage = loadPage("manager/mediawiki_install.html")

// InstallPageData is manager/mediawiki_install.html's template context.
...
func renderInstallPage(a *appctx.App, w http.ResponseWriter, r *http.Request, domains []appctx.Domain) {
	layout, _, err := web.BuildLayoutData(a, w, r, "Install MediaWiki")
	if err != nil {
	...
	if err := installPage.Render(w, http.StatusOK, data); err != nil {
		log.Printf("MEDIAWIKI - install template render error: %v", err)
	}
}
