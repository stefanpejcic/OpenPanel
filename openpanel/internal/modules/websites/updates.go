...
// latestVersionWithZipAsset mirrors prestashop/version.go's and matomo/version.go's "only count a release that actually ships a downloadable asset" filter
...
// latestMediaWikiVersionForUpdates scrapes releases.wikimedia.org/mediawiki/ (a two-level branch/patch directory listing, not a GitHub-tags API - see mediawiki/version.go's identical scraper) for the highest patch version off the highest available branch
func latestMediaWikiVersionForUpdates(ctx context.Context) (string, error) {
	fetch := func(url string) (string, error) {
		...
		// releases.wikimedia.org returns 403 to Go's default "Go-http-client" User-Agent - confirmed live, a plain browser/curl-like UA is required
...
// latestWordPressVersionForUpdates asks wordpress.org's own stable-check API for the current stable release (the same source list.html's client-side badge already reads per-version, just used here to get the single latest tag server-side)
...
// latestVersionsForAllTypes fetches every supported CMS's latest version concurrently and returns a lowercase-type -> version map, memoized for 6 hours since these barely change and this is 8 outbound API calls
