...
type prestashopBackupDateInfo struct {
...
func handlePrestashopGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []prestashopBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !prestashopBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := prestashopBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handlePrestashopRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !prestashopBackupDateRE.MatchString(backupDate) {
	...
	if _, statErr := os.Stat(targzPathOnHostOS); statErr == nil {
		...
		_ = logger.RecordUserAction(a.Config, currentUsername, "restored PrestaShop files backup from "+backupDatePathInContainer+" on "+selectedDomain, ipAddress)
	...
	if _, statErr := os.Stat(databaseSQLPathOnHostOS); statErr == nil {
		...
		if dbName != "" {
			...
			_ = logger.RecordUserAction(a.Config, currentUsername, "restored PrestaShop database backup from "+databaseSQLPathInContainer+" on "+selectedDomain, ipAddress)
...
func handlePrestashopRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if backupDatabase {
		...
		if dbName == "" || tablePrefix == "" {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to determine database name/prefix from app/config/parameters.php."})
			return
		}
	...
	case backupDatabase && backupFiles:
		logAction = "generated full backup for PrestaShop website " + selectedDomain
	case backupDatabase:
		logAction = "generated database backup for PrestaShop website " + selectedDomain
	case backupFiles:
		logAction = "generated files backup for PrestaShop website " + selectedDomain
	}
