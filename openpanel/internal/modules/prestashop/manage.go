...
func handleRemovePrestashop(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	row := a.DB.QueryRowContext(ctx, `
		...
		WHERE sites.id = ? AND sites.type = 'prestashop'`, id)
	...
	if docroot == "" {
		flashAndRedirect(a, w, r, "error", "PrestaShop installation not found in the database", "/sites")
		return
	}
	...
	configFile := filepath.Join(volume, "app", "config", "parameters.php")
	...
	if dbNameMatch != nil && dbUserMatch != nil {
		...
		flashSess(a, w, r, "warning", "Database name or user not found in app/config/parameters.php")
	}
	...
	if _, delErr := a.DB.ExecContext(ctx, "DELETE FROM sites WHERE id = ?", id); delErr != nil {
		flashSess(a, w, r, "error", "An error occurred during PrestaShop uninstall.")
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": delErr.Error()})
		return
	}

	_ = logger.RecordUserAction(a.Config, currentUsername, "uninstalled PrestaShop website "+siteName, reqip.ClientIP(r))

	if r.URL.Query().Get("output") == "json" {
		writeJSON(w, http.StatusOK, map[string]string{"message": "PrestaShop uninstalled successfully"})
		return
	}
	flashAndRedirect(a, w, r, "success", "PrestaShop uninstalled successfully", "/sites")
}
