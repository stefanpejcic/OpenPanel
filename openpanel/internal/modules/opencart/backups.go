...
var opencartBackupFolderRE = regexp.MustCompile(`^20\d{2}-`)

type opencartBackupDateInfo struct {
...
func handleOpenCartGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []opencartBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !opencartBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := opencartBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handleOpenCartRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !opencartBackupDateRE.MatchString(backupDate) {
	...
	if _, statErr := os.Stat(targzPathOnHostOS); statErr == nil {
		...
		_ = logger.RecordUserAction(a.Config, currentUsername, "restored OpenCart files backup from "+backupDatePathInContainer+" on "+selectedDomain, ipAddress)
		restoredItems = append(restoredItems, "files")
	}

	if _, statErr := os.Stat(databaseSQLPathOnHostOS); statErr == nil {
		dbInfo := opencartDBInfoForBackup(userContext, docroot)
		...
		if dbName != "" {
			...
			_ = logger.RecordUserAction(a.Config, currentUsername, "restored OpenCart database backup from "+databaseSQLPathInContainer+" on "+selectedDomain, ipAddress)
...
func handleOpenCartRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	case backupDatabase && backupFiles:
		logAction = "generated full backup for OpenCart website " + selectedDomain
	case backupDatabase:
		logAction = "generated database backup for OpenCart website " + selectedDomain
	case backupFiles:
		logAction = "generated files backup for OpenCart website " + selectedDomain
	}
