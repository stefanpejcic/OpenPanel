...
func handleRemoveOpenCart(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	row := a.DB.QueryRowContext(ctx, `
		...
		WHERE sites.id = ? AND sites.type = 'opencart'`, id)
	...
	if docroot == "" {
		flashAndRedirect(a, w, r, "error", "OpenCart installation not found in the database", "/sites")
		return
	}
	...
	if _, delErr := a.DB.ExecContext(ctx, "DELETE FROM sites WHERE id = ?", id); delErr != nil {
		flashSess(a, w, r, "error", "An error occurred during OpenCart uninstall.")
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": delErr.Error()})
		return
	}

	_ = logger.RecordUserAction(a.Config, currentUsername, "uninstalled OpenCart website "+siteName, reqip.ClientIP(r))

	if r.URL.Query().Get("output") == "json" {
		writeJSON(w, http.StatusOK, map[string]string{"message": "OpenCart uninstalled successfully"})
		return
	}
	flashAndRedirect(a, w, r, "success", "OpenCart uninstalled successfully", "/sites")
}
