// Package opencart installs and manages an OpenCart site (downloaded from
// GitHub releases + OpenCart's own CLI installer,
// `install/cli_install.php install`) inside an existing domain's docroot,
// run in the domain's existing php-fpm container - same shape as
// internal/modules/joomla. Deliberately minimal, matching drupal/joomla's
// scope: no cloning, no scan-for-existing-installs, no hardening rules, no
// dedicated backup/restore system - just install, a small read-only
// manage/overview page (files, database, PHP/DB versions, screenshot), a
// Logs tab, cache clearing, a one-time admin login link, and uninstall.
// MySQL/MariaDB only.
package opencart
...
// credentials/tokens (same approach as joomla.generateRandomString).
...
// operation per user at a time - not an OpenCart-specific lock.
...
// wordpress/phpapp/drupal/joomla each duplicate locally rather than sharing
