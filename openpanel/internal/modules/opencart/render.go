...
var installPage = loadPage("manager/opencart_install.html")

// InstallPageData is manager/opencart_install.html's template context.
...
func renderInstallPage(a *appctx.App, w http.ResponseWriter, r *http.Request, domains []appctx.Domain) {
	layout, _, err := web.BuildLayoutData(a, w, r, "Install OpenCart")
	if err != nil {
	...
	if err := installPage.Render(w, http.StatusOK, data); err != nil {
		log.Printf("OPENCART - install template render error: %v", err)
	}
}
