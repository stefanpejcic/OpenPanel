...
type joomlaBackupDateInfo struct {
...
func handleJoomlaGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []joomlaBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !joomlaBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := joomlaBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handleJoomlaRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !joomlaBackupDateRE.MatchString(backupDate) {
	...
	if _, statErr := os.Stat(targzPathOnHostOS); statErr == nil {
		...
		_ = logger.RecordUserAction(a.Config, currentUsername, "restored Joomla files backup from "+backupDatePathInContainer+" on "+selectedDomain, ipAddress)
	...
	if _, statErr := os.Stat(databaseSQLPathOnHostOS); statErr == nil {
		...
		if dbName != "" {
			...
			_ = logger.RecordUserAction(a.Config, currentUsername, "restored Joomla database backup from "+databaseSQLPathInContainer+" on "+selectedDomain, ipAddress)
...
// handleJoomlaRunBackup mirrors wordpress/backups.go's handleRunBackup.
func handleJoomlaRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	selectedDomain := r.PathValue("selected_domain")
	docroot := r.URL.Query().Get("docroot")
	if docroot == "" {
	...
	if backupDatabase {
		...
		if dbName == "" || tablePrefix == "" {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to determine database name/prefix from configuration.php."})
			return
		}
	...
	case backupDatabase && backupFiles:
		logAction = "generated full backup for Joomla website " + selectedDomain
	case backupDatabase:
		logAction = "generated database backup for Joomla website " + selectedDomain
	case backupFiles:
		logAction = "generated files backup for Joomla website " + selectedDomain
	}
