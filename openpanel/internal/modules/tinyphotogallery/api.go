...
// apiInstallTinyPhotoGallery delegates straight to handleInstallPage (calls handleInstallStream on POST): same site-limit check, same NDJSON progress stream, just fed from the API's JSON body instead of a UI form post
...
// apiRemoveTinyPhotoGallery delegates to handleRemoveTinyPhotoGallery with the path's {site_id} translated into the "id" form field it expects, and output=json forced so it returns JSON instead of a flash-and-redirect
