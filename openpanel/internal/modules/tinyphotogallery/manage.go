...
func handleRemoveTinyPhotoGallery(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	row := a.DB.QueryRowContext(ctx, `
		...
		WHERE sites.id = ? AND sites.type = 'tinyphotogallery'`, id)
	...
	if docroot == "" {
		flashAndRedirect(a, w, r, "error", "TinyPhotoGallery installation not found in the database", "/sites")
		return
	}
	...
	if _, delErr := a.DB.ExecContext(ctx, "DELETE FROM sites WHERE id = ?", id); delErr != nil {
		flashSess(a, w, r, "error", "An error occurred during TinyPhotoGallery uninstall.")
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": delErr.Error()})
		return
	}

	_ = logger.RecordUserAction(a.Config, currentUsername, "uninstalled TinyPhotoGallery website "+siteName, reqip.ClientIP(r))

	if r.URL.Query().Get("output") == "json" {
		writeJSON(w, http.StatusOK, map[string]string{"message": "TinyPhotoGallery uninstalled successfully"})
		return
	}
	flashAndRedirect(a, w, r, "success", "TinyPhotoGallery uninstalled successfully", "/sites")
}
