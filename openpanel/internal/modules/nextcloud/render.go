...
var installPage = loadPage("manager/nextcloud_install.html")

// InstallPageData is manager/nextcloud_install.html's template context.
...
func renderInstallPage(a *appctx.App, w http.ResponseWriter, r *http.Request, domains []appctx.Domain) {
	layout, _, err := web.BuildLayoutData(a, w, r, "Install Nextcloud")
	if err != nil {
	...
	if err := installPage.Render(w, http.StatusOK, data); err != nil {
		log.Printf("NEXTCLOUD - install template render error: %v", err)
	}
}
