...
type nextcloudBackupDateInfo struct {
...
func handleNextcloudGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []nextcloudBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !nextcloudBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := nextcloudBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handleNextcloudRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !nextcloudBackupDateRE.MatchString(backupDate) {
	...
	if _, statErr := os.Stat(targzPathOnHostOS); statErr == nil {
		...
		_ = logger.RecordUserAction(a.Config, currentUsername, "restored Nextcloud files backup from "+backupDatePathInContainer+" on "+selectedDomain, ipAddress)
	...
	if _, statErr := os.Stat(databaseSQLPathOnHostOS); statErr == nil {
		...
		if dbName != "" {
			...
			_ = logger.RecordUserAction(a.Config, currentUsername, "restored Nextcloud database backup from "+databaseSQLPathInContainer+" on "+selectedDomain, ipAddress)
...
func handleNextcloudRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if backupDatabase {
		...
		if dbName == "" || tablePrefix == "" {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to determine database name/prefix from config/config.php."})
			return
		}
	...
	case backupDatabase && backupFiles:
		logAction = "generated full backup for Nextcloud website " + selectedDomain
	case backupDatabase:
		logAction = "generated database backup for Nextcloud website " + selectedDomain
	case backupFiles:
		logAction = "generated files backup for Nextcloud website " + selectedDomain
	}
