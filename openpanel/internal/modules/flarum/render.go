...
var installPage = loadPage("manager/flarum_install.html")

// InstallPageData is manager/flarum_install.html's template context.
...
func renderInstallPage(a *appctx.App, w http.ResponseWriter, r *http.Request, domains []appctx.Domain) {
	layout, _, err := web.BuildLayoutData(a, w, r, "Install Flarum")
	if err != nil {
	...
	if err := installPage.Render(w, http.StatusOK, data); err != nil {
		log.Printf("FLARUM - install template render error: %v", err)
	}
}
