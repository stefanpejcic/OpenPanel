...
// handleFlarumClone mirrors drupal/clone.go's handleDrupalClone.
func handleFlarumClone(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	dstDB := strings.ToLower(formOr(r, "target_db", "flarum_clone_"+generateRandomString(6)))
	...
	if mkErr := os.MkdirAll(dstPath, 0o755); mkErr != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to copy Flarum files: " + mkErr.Error()})
		return
	}
	if cpErr := exec.CommandContext(ctx, "cp", "-a", srcPath+"/.", dstPath+"/").Run(); cpErr != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "Failed to copy Flarum files: " + cpErr.Error()})
		return
	}
	...
	strContent = cloneFlarumDatabaseRE.ReplaceAllString(strContent, "'database' => '"+escapePHPSingleQuoted(dstDB)+"',")
	strContent = cloneFlarumUsernameRE.ReplaceAllString(strContent, "'username' => '"+escapePHPSingleQuoted(dstDBUser)+"',")
	strContent = cloneFlarumPasswordRE.ReplaceAllString(strContent, "'password' => '"+escapePHPSingleQuoted(dstDBUserPassword)+"',")
	strContent = cloneFlarumURLRE.ReplaceAllString(strContent, "'url' => 'https://"+escapePHPSingleQuoted(dstDomainWithSubdir)+"',")
	...
	if !strings.Contains(strContent, dstDB) {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"status": "error", "step": "Failed to set 'database' in config.php"})
		return
	}

	adminEmail := formOr(r, "admin_email", "admin@"+dstDomain)
	flarumVersion := formOr(r, "flarum_version", "latest")
