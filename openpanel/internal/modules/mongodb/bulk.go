...
func databasesBulkActions(t i18n.Translator, users []string) []web.BulkAction {
	return []web.BulkAction{
		{Key: "export", Label: t.Get("Export"), Confirm: t.Get("Export the selected databases as .archive.gz files to this folder:"),
			Input: &web.BulkInput{Type: "text", Default: dbexport.WebRoot, Placeholder: dbexport.WebRoot}},
		{Key: "assign", Label: t.Get("Assign user"), Confirm: t.Get("Give this user read and write access to the selected databases:"), Input: &web.BulkInput{Type: "select", Options: nameOptions(users)}},
...
func usersBulkActions(t i18n.Translator, databases []string) []web.BulkAction {
	return []web.BulkAction{
		...
		{Key: "assign", Label: t.Get("Add to database"), Confirm: t.Get("Give the selected users read and write access to:"), Input: &web.BulkInput{Type: "select", Options: nameOptions(databases)}},
...
// handleBulk serves both /mongodb/bulk (items are databases) and /mongodb/users/bulk (items are users)
func handleBulk(a *appctx.App, mux http.Handler, w http.ResponseWriter, r *http.Request, usersPage bool) {
	...
	web.ServeBulkDispatch(a, mux, w, r, actions, func(action, value, item string) (*web.BulkCall, web.BulkResult) {
		...
		switch {
		case action == "export":
			return web.Call(web.BulkCall{Method: http.MethodPost, Path: "/mongodb/export", Form: url.Values{
				"database_name": {db}, "export_destination": {"files"}, "export_format": {"gzip"}, "local_path": {value},
			}})
		case action == "assign":
			return web.Call(web.BulkCall{Method: http.MethodPost, Path: "/mongodb/assign", Form: url.Values{"db_user": {user}, "database_name": {db}}})
		case action == "unassign":
			return web.Call(web.BulkCall{Method: http.MethodPost, Path: "/mongodb/remove_user", Form: url.Values{"db_user": {user}, "database_name": {db}}})
		case action == "password":
			return web.Call(web.BulkCall{Method: http.MethodPost, Path: "/mongodb/change_user_password", Form: url.Values{"db_user": {item}, "new_password": {value}}})
		case action == "delete" && usersPage:
			return web.Call(web.BulkCall{Method: http.MethodPost, Path: "/delete_mongodb_user", Form: url.Values{"db_user": {item}}})
		case action == "delete":
			return web.Call(web.BulkCall{Method: http.MethodPost, Path: "/mongodb/delete", Form: url.Values{"database_name": {item}}})
		}
