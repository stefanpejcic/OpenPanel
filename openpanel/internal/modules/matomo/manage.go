...
func handleRemoveMatomo(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	row := a.DB.QueryRowContext(ctx, `
		...
		WHERE sites.id = ? AND sites.type = 'matomo'`, id)
	...
	if docroot == "" {
		flashAndRedirect(a, w, r, "error", "Matomo installation not found in the database", "/sites")
		return
	}
	...
	configFile := filepath.Join(volume, "config", "config.ini.php")
	...
	if dbNameMatch != nil && dbUserMatch != nil {
		...
		flashSess(a, w, r, "warning", "Database name or user not found in config/config.ini.php")
	}

	removeMatomoCredentials(siteName)
	...
	if _, delErr := a.DB.ExecContext(ctx, "DELETE FROM sites WHERE id = ?", id); delErr != nil {
		flashSess(a, w, r, "error", "An error occurred during Matomo uninstall.")
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": delErr.Error()})
		return
	}

	_ = logger.RecordUserAction(a.Config, currentUsername, "uninstalled Matomo website "+siteName, reqip.ClientIP(r))

	if r.URL.Query().Get("output") == "json" {
		writeJSON(w, http.StatusOK, map[string]string{"message": "Matomo uninstalled successfully"})
		return
	}
	flashAndRedirect(a, w, r, "success", "Matomo uninstalled successfully", "/sites")
}
