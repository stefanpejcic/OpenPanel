...
// openpanelLoginFileName is the login helper's filename inside the Matomo docroot - deployed once at install time (install.go) and used by handleMatomoLogin (cli.go) to build the link the browser opens
const openpanelLoginFileName = "openpanel-login.php"

// buildOpenpanelLoginPHP renders the login helper script with the admin login/password/secret token baked in as literals - Matomo ships no CLI/DB-level one-time-login primitive the way Drush's `user:login` or a plain token table (joomla/opencart/nextcloud/prestashop) can drive, so rather than reimplement its internal session bootstrap, this script replays the exact same public login form flow (nonce fetch, then POST to module=Login) server-side via a loopback cURL call, then re-issues whatever session cookie that exchange produced as a genuine Set-Cookie on its own response so the real browser ends up holding a real Matomo session - protected by a random 64-hex-char token baked in at install time, same trust boundary as any other CMS module's config file sitting in the docroot with a plaintext DB password; the loopback request targets http://<webServerContainer>/... with an explicit Host header rather than the site's own public HTTPS URL, since php-fpm containers here have no outbound route back to the server's own public IP/port 443, but a plain HTTP request to the webserver container's own name on the shared podman network reaches it directly
...
// phpStringLiteral renders s as a single-quoted PHP string literal, escaping backslashes and single quotes (the only two characters that matter inside PHP single-quoted strings)
