...
var dokuwikiBackupFolderRE = regexp.MustCompile(`^20\d{2}-`)

type dokuwikiBackupDateInfo struct {
	Date           string `json:"date"`
	HasFilesBackup bool   `json:"hasFilesBackup"`
}

func handleDokuwikiGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []dokuwikiBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !dokuwikiBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := dokuwikiBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
var dokuwikiBackupDateRE = regexp.MustCompile(`^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}$`)

func handleDokuwikiRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	selectedDomain := r.PathValue("selected_domain")
	...
	if !dokuwikiBackupDateRE.MatchString(backupDate) {
	...
	_ = logger.RecordUserAction(a.Config, currentUsername, "restored DokuWiki files backup from "+backupDatePathInContainer+" on "+selectedDomain, reqip.ClientIP(r))
	_, _ = w.Write([]byte("Backup restored successfully: files."))
}

func handleDokuwikiRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	selectedDomain := r.PathValue("selected_domain")
	docroot := r.URL.Query().Get("docroot")
	if docroot == "" {
	...
	_ = logger.RecordUserAction(a.Config, currentUsername, "generated files backup for DokuWiki website "+selectedDomain, reqip.ClientIP(r))
