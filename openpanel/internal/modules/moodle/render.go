...
var installPage = loadPage("manager/moodle_install.html")

// InstallPageData is manager/moodle_install.html's template context.
...
func renderInstallPage(a *appctx.App, w http.ResponseWriter, r *http.Request, domains []appctx.Domain) {
	layout, _, err := web.BuildLayoutData(a, w, r, "Install Moodle")
	if err != nil {
	...
	if err := installPage.Render(w, http.StatusOK, data); err != nil {
		log.Printf("MOODLE - install template render error: %v", err)
	}
}
