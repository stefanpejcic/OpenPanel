...
// Register wires the Moodle install/remove/manage routes onto mux, gated behind the "moodle" feature flag - no list page, matches drupal/joomla/opencart/nextcloud/prestashop's scope of managing via the general Site Manager instead
...
// withMoodleForm clones r as a POST carrying values as both Form and PostForm, so a UI handler reading r.FormValue(...) sees the API's JSON body fields - same pattern used by every other CMS module's with{CMS}Form
