...
var sofawikiBackupFolderRE = regexp.MustCompile(`^20\d{2}-`)

type sofawikiBackupDateInfo struct {
...
func handleSofawikiGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []sofawikiBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !sofawikiBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := sofawikiBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handleSofawikiRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !sofawikiBackupDateRE.MatchString(backupDate) {
	...
	_ = logger.RecordUserAction(a.Config, currentUsername, "restored SofaWiki files backup from "+backupDatePathInContainer+" on "+selectedDomain, reqip.ClientIP(r))
...
func handleSofawikiRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	_ = logger.RecordUserAction(a.Config, currentUsername, "generated files backup for SofaWiki website "+selectedDomain, reqip.ClientIP(r))
