...
var tinyfilemanagerBackupFolderRE = regexp.MustCompile(`^20\d{2}-`)

type tinyfilemanagerBackupDateInfo struct {
...
func handleTinyFileManagerGetBackupDates(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	var dates []tinyfilemanagerBackupDateInfo
	for _, entry := range entries {
		if !entry.IsDir() || !tinyfilemanagerBackupFolderRE.MatchString(entry.Name()) {
			continue
		}
		folderPath := filepath.Join(backupsPath, entry.Name())
		files, _ := os.ReadDir(folderPath)
		info := tinyfilemanagerBackupDateInfo{Date: entry.Name()}
		for _, f := range files {
...
func handleTinyFileManagerRestoreBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	if !tinyfilemanagerBackupDateRE.MatchString(backupDate) {
	...
	_ = logger.RecordUserAction(a.Config, currentUsername, "restored TinyFileManager files backup from "+backupDatePathInContainer+" on "+selectedDomain, reqip.ClientIP(r))
...
func handleTinyFileManagerRunBackup(a *appctx.App, w http.ResponseWriter, r *http.Request) {
	...
	_ = logger.RecordUserAction(a.Config, currentUsername, "generated files backup for TinyFileManager website "+selectedDomain, reqip.ClientIP(r))
