...
func Register(mux *http.ServeMux, a *appctx.App) {
	...
	mux.Handle("POST /wordpress/backup/restore/{selected_domain...}", requireLogin(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		handleRestoreBackup(a, w, r)
	})))
	mux.Handle("POST /wordpress/backup/run/{selected_domain...}", requireLogin(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	mux.Handle("POST /wordpress/reload_data", requireLogin(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		handleReloadWordPressData(a, w, r)
	})))
	mux.Handle("POST /wordpress/scan", requireLogin(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
